...
using Loja.Venda.Dominio.Enum;
using Loja.Venda.Dominio.Interfaces;
using Microsoft.EntityFrameworkCore;

namespace Loja.Venda.Data.Repository
{
    public class PedidoRepository : IPedidoRepository
    {
        ...
        public async Task<Pedido> ObterPedidoRascunhoPorClienteId(Guid clienteId)
        {
            var pedido = await _context.Pedidos.FirstOrDefaultAsync(
                p => p.ClienteId == clienteId && p.PedidoStatus == PedidoStatus.Rascunho
            );
        ...
        public async Task<PedidoItem> ObterItemPorPedido(Guid pedidoId, Guid produtoId)
        {
            return await _context.PedidoItems.FirstOrDefaultAsync(
                p => p.ProdutoId == produtoId && p.PedidoId == pedidoId
            );
        }
